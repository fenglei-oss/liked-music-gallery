...
.cover-field img:hover {
  ...
  z-index: 10;
  transition: transform 1.5s cubic-bezier(.25,.1,.25,1), box-shadow .3s ease, z-index 0s;
}
.toolbar {
  position: sticky;
  top: 0;
...
.track-body { padding: 16px 16px 12px; flex: 1; display: flex; flex-direction: column; }
...
.player-wrap { padding: 0 6px 8px; }
.player-frame { width: 100%; height: 66px; border: 0; border-radius: 12px; overflow: hidden; }
...
.list .player-wrap { grid-column: 4; padding: 8px; }
.list .player-frame { width: 220px; height: 52px; }
.empty {
  display: none;
  padding: 54px 20px;
  text-align: center;
...
const trackMarkup = (track) => \`
  <article class="track">
    ...
    <div class="player-wrap">
      <iframe class="player-frame" src="https://music.163.com/outchain/player?type=2&id=\${track.id}&auto=0&height=66" frameborder="0" width="100%" height="66" loading="lazy" allow="autoplay"></iframe>
    </div>
  </article>\`;
